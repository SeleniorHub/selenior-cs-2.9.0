...
// Mappers — Clientes: id|nome|nicho|fase|churn|dataInicio|mrr|indicador|comissaoVal|comissaoTipo|checkpoints|done|nota|depoimento
function clientToRow(cl){return[cl.id,cl.nome,cl.nicho,cl.fase,cl.churn,cl.dataInicio||'',cl.mrr||'0',cl.indicador||'',cl.comissaoVal||'',cl.comissaoTipo||'pct',(cl.checkpoints||[]).join('||'),(cl.done||[]).join('||'),cl.nota||'',cl.depoimento||''];}
function rowToClient(r){return{id:String(r[0]),nome:r[1]||'',nicho:r[2]||'',fase:r[3]||'Onboarding',churn:r[4]||'baixo',dataInicio:r[5]||'',mrr:String(r[6]||'0'),indicador:r[7]||'',comissaoVal:r[8]||'',comissaoTipo:r[9]||'pct',checkpoints:r[10]?String(r[10]).split('||').filter(Boolean):[],done:r[11]?String(r[11]).split('||').filter(Boolean):[],nota:r[12]||'',depoimento:r[13]||''};}
