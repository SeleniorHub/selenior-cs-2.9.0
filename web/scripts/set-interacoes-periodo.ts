...
//
// Uso: npx tsx scripts/set-interacoes-periodo.ts <webhook_slug> <data_inicio:YYYY-MM-DD> <data_fim:YYYY-MM-DD> <interacoes>
// Exemplo: npx tsx scripts/set-interacoes-periodo.ts chic-estofados 2026-06-01 2026-06-30 2699
...
async function main() {
  const [slug, dataInicio, dataFim, interacoesStr] = process.argv.slice(2);
  if (!slug || !dataInicio || !dataFim || !interacoesStr) {
    throw new Error("uso: tsx scripts/set-interacoes-periodo.ts <webhook_slug> <data_inicio:YYYY-MM-DD> <data_fim:YYYY-MM-DD> <interacoes>");
  }
  ...
  for (const [i, data] of days.entries()) {
    ...
    await db
      ...
      .values({ accountId: account.id, data, interacoes })
      .onConflictDoUpdate({
  ...
  console.log(`${account.nome}: ${interacoesTotal} interações distribuídas em ${days.length} dias (${dataInicio} a ${dataFim}).`);
